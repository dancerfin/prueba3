...
DETECTION_LOG = "ddos_detection.log"  # Archivo de registro
...
def log_detection_status():
    global TOTAL_ATTACKS, DETECTED_ATTACKS
    ...
    log_entry = (f"DDoS Detection Rate: {detection_rate:.2f}% | "
    ...
    MONITOR_STATS['last_detection_log'] = current_time
...
class DDoSML(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        with open(DETECTION_LOG, 'w') as f:
            f.write("=== Inicio de registro de detecciones DDoS ===\n")
    ...
    def _flow_monitor(self):
        ...
        while True:
            ...
            # Registrar estado de detección cada minuto
            if current_time - MONITOR_STATS['last_detection_log'] >= 60:
    ...
    def _is_real_attack(self, dpid, result):
        ...
        if '1' in result:
            ...
            if not status['active']:
                ...
                status['active'] = True
                ATTACK_START_TIMES[dpid] = time.time()
                TOTAL_ATTACKS += 1
        ...
        if (duration >= MIN_ATTACK_DURATION and 
            ...
            detection_time = time.time() - ATTACK_START_TIMES.get(dpid, time.time())
            DETECTION_TIMES.append(detection_time)
            DETECTED_ATTACKS += 1
            
            self.logger.info(f"Switch {dpid}: Ataque confirmado. Tiempo de detección: {detection_time:.2f}s")
            log_detection_status()
            return True
    ...
    def _limit_new_flows(self, datapath):
        ...
        # Configurar medidor para limitar tráfico
        meter_id = 1
        ...
        # Crear flujo que aplica el medidor
        match = parser.OFPMatch()
        instructions = [
